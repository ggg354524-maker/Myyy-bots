...
words_list = ["كس امك", "يابن الشرموطه", "يابن الضعيفتين", "يابن الشرموطتين", "يابن القحبتين", "ي ابن الديوث", "كس اختك", "ادعس امك", "ياكس امك", "ي ابن الضعيفه", "ادوس مامتك", "كس عيلتك", "ي ابن العاهرتين", "ي ابن المصخرتين", "يا هطف", "ي مصخره", "مضحكه"]
        ...
        if event.raw_text.startswith("اصمل "):
            ...
            while is_running:
                ...
                if current_time - last_enemy_time >= 15:
                    try: await client.send_message(target_chat, f"ي {enemy_name} {random.choice(words_list)} 🦋"); last_enemy_time = current_time
                    except: pass
                if current_time - last_special_time >= 10:
                    random.shuffle(special_words)
                    try: await client.send_message(target_chat, f"{' '.join(special_words)} {random.choice(emojis)}"); last_special_time = current_time
                    except: pass
                while True:
                    selected = random.sample(words_list, 3)
                    new_line = " ".join(selected)
                    if new_line != last_sent_line: last_sent_line = new_line; break
                try:
                ...
                except: await asyncio.sleep(speed)
